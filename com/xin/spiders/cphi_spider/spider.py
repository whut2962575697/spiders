...
class DownloadTool(object):
    ...
    def download_page(self, urlmd5, url, proxy):
        ...
        page = downloader.simple_download(url=url)
        if page is not None:
            company_name = self.extract_field_from_page(page=page,reg=r"""<meta name="keywords" content="([^=]+)"/>""",index=-1)
            company_net = self.extract_field_from_page(page=page, reg=r'"/companyurlimg.php\?url=([^<]+)" alt="myImage" style="border:none')
            address = self.extract_field_from_page(page=page.decode("utf-8"), reg="<p>([^=]+)</p>",index=-1)
            ...
            db = MysqlHandle()
            db.insert(sql=sql, value_list=[(urlmd5, url, company_name,company_net,address)])
            db.close()
            update_sql = "update " + self.url_table + " set status=200 where urlmd5='%s'" % (urlmd5)
            db = MysqlHandle()
            db.update(sql=update_sql)
    ...
    def extract_field_from_page(self, page, reg,index=0):
