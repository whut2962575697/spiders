...
class Spider(object):
    ...
    def process(self, tuple_from_queue, proxy):
        ...
        if url_type != 0:
            res = download_tool.download_list_page(urlmd5=urlmd5, url=url, proxy=proxy, domain="https://www.sq688.com")
...
class DownloadTool(object):
    ...
    def download_page(self, urlmd5, url, proxy):
        ...
        page = downloader.simple_download(url=url)
        if page is not None:
            file_name = self.extract_field_from_page(page=page, reg=r'<h2>([^\]]+)    				<span class="c?b?">'
                                                                    r'WAV</span>')
            # if file_name is None:
            #     file_name = self.extract_field_from_page(page=page, reg=r'<h1>([^<]+)下载?</h1>')
            singer_name = self.extract_field_from_page(page=page, reg=r'<p><a href="/detail/\d+.html" style="color:'
                                                                      r'#217fbc">([^=]+)</a></p>')
            result = self.extract_field_from_page(page=page, reg=r'<p class="downurl">链接: (https?://pan.baidu.com/s/[^=]+) 密码: (\w+)</p>')
