...
class _360MapSpider(object):
    def __init__(self, keyword,num):
        self.keyword = keyword
        self.num = num
        self.base_url = "http://restapi.map.so.com/api/simple?resType=json&mobile=1&flag=callback&encode=UTF-8&keyword=%s&city=北京&cityname=北京&cityid=undefined&batch=%d&number=10&sid=1000&ext=1&qii=true&routePoint=&routeType=0&routeSelectPOI=0"

    def download_page_info(self,json_data):
        if json_data.has_key("poi"):
            content = json_data["poi"]
            value_list = []
            for item in content:
...
if __name__ == "__main__":
    ...
    spider = _360MapSpider(args.keyword.decode("gbk").encode("utf-8"),int(args.num))
    spider.spider()
    # spider = _360MapSpider("郑州市广告公司",20)
    # spider.spider()